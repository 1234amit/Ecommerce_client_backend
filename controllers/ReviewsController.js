...
export const createReview = async (req, res) => {
  try {
    const { userName, rating, comment, productId, role } = req.body;

    if (!userName || !rating || !comment || !productId || !role) {
      return res.status(400).json({ message: "All fields are required" });
    }
...
export const getReviewsByProduct = async (req, res) => {
  try {
    ...
    const [reviews, stats] = await Promise.all([
      Review.find(filter)
        .sort({ createdAt: -1 })
        .select("userName rating comment createdAt role")
        .lean(),
...
export const getReviewsByUserName = async (req, res) => {
  try {
    ...
    const aggregation = [
      ...
      ...(pid ? [{ $match: { productId: pid } }] : []),
      {
        $group: {
          _id: pid ? "$productId" : "$userName",
          totalReviews: { $sum: 1 },
          averageRatingGiven: { $avg: "$rating" },
        },
      },
    ...
    return res.status(200).json({
      ...
      averageRatingGiven: stats[0]?.averageRatingGiven
        ? Number(stats[0].averageRatingGiven.toFixed(1))
        : 0,
      reviews,
    });
  } catch (err) {
    console.error("❌ getReviewsByUserName error:", err);
    return res.status(500).json({ message: "Internal server error" });
  }
};
